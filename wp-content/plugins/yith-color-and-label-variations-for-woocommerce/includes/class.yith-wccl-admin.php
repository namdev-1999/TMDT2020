...
if ( ! class_exists( 'YITH_WCCL_Admin' ) ) {
	...
	class YITH_WCCL_Admin {
		...
		public function print_attribute_type( $attribute, $value = '', $table = 0 ) {
			...
			if ( $table ): ?>
				<tr class="form-field">
				<th scope="row" valign="top"><label for="term-value"><?php echo esc_html( $custom_types[ $type ] ); ?></label></th>
				<td>
			<?php else: ?>
		...
		function product_option_terms( $tax, $i ) {
			...
			if ( in_array( $tax->attribute_type, array( 'colorpicker', 'image', 'label' ) ) ) {
				...
				<select multiple="multiple"
					data-placeholder="<?php _e( 'Select terms', 'yith-color-and-label-variations-for-woocommerce' ); ?>"
				...
				<button
					class="button plus select_all_attributes"><?php _e( 'Select all', 'yith-color-and-label-variations-for-woocommerce' ); ?></button>
				<button
					class="button minus select_no_attributes"><?php _e( 'Select none', 'yith-color-and-label-variations-for-woocommerce' ); ?></button>
				<button class="button fr plus add_new_attribute"
					data-attribute="<?php echo $attribute_taxonomy_name; ?>"><?php _e( 'Add new', 'yith-color-and-label-variations-for-woocommerce' ); ?></button>
				<?php
			}
		}
		...
		public function register_panel() {
			...
			$args = array(
				...
				'parent_slug'      => '',
				'page_title'       => _x( 'WooCommerce Color and Label Variations', 'plugin name in admin page title', 'yith-color-and-label-variations-for-woocommerce' ),
		...
		public function customize_premium_banner_link( $banners ) {
			$banners['upgrade']['link'] = $this->premium_landing_url;
			return $banners;
		}
